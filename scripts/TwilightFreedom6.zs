...
events.onPlayerInteractEntity(function(event as PlayerInteractEntityEvent) {
  ...
  if (!isNull(heldItem) && heldItem.definition.id.matches(<contenttweaker:crimson_sundering_pebble>.definition.id)) {
    ...
    if ((target.definition.id != "twilightforest:raven") || !target.isAlive()) {
      return;
    }  
    ...
    if (target.health <= DAMAGE) {
      ...
      // give custom drop
      Commands.call("summon villager ~ ~3 ~ {Invulnerable:1, CustomName:\"Twilit Dweller\",Offers:{Recipes:[{buy:{id:\"twilightforest:magic_map_focus\",Count:64},sell:{id:\"contenttweaker:whisper_of_freedom_raven\",Count:1},rewardExp:0b,maxUses:9999999}]},Profession:2,Career:1,CareerLevel:3,ActiveEffects:[{Id:14,Amplifier:0,Duration:999999,ShowParticles:0b},{Id:24,Amplifier:0,Duration:999999,ShowParticles:0b}]}", event.player, event.world, false, true);
